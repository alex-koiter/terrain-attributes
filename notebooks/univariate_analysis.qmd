...
all_terrain <- ag_data |>
  ...
  rename("Terrain Attribute" = name, Mean = mean, SD = sd, Max = max, Min = min, Skewness = skewness, CV = cv)
...
#| tbl-cap: Summary univariate statistics of geochemical and colour soil properties for each site (n = 49). Geochemical concentrations are reported in ppm, excecpt Ca and Fe (%).
...
# all_fingerprints |>
#   #filter(Skewness >= 0.5 | Skewness <= -0.5) |>
#   filter(CV >= 35)|>
#   filter(site == "Forest")
#   #filter(group %in% c("geochem", "colour")) |>
#   group_by(site, group) |>
...
#| tbl-cap: Pearson’s correlation coefficients for soil colour and geochemical properties and terrain attributes (n=49).
