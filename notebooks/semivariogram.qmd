...
vario <- read_ods(here::here("./notebooks/semivariogram.ods")) %>%
  ...
  #mutate(Property = (fct_recode(Property, "*h**" = "h*", "*a**" = "a*", "*c**" = "c*", "*b**" = "b*", "*x*" = "x"))) %>%
  #select(-p_sill) %>%
...
#| tbl-cap: Geostatistical parameters of the fitted semivariogram models of colour and geochemical properties within the agricultural and forested sites.
...
vario |>
  ...
  cols_label(r2 = "{{r^2}}") |>
  tab_footnote(
    footnote = "Stable model type was used for all analysis. Models are all isotropic.",
    locations = cells_column_labels(columns = "Kriging Type"))  |>
  tab_footnote(
    footnote = "Cross-validation of the measured and predicted values",
